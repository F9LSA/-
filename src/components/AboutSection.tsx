...
export default function AboutSection({ showWhyChoose = false }: AboutSectionProps) {
  ...
  return (
    <section
      id="about"
      className="relative min-h-screen flex flex-col items-center justify-center px-4 sm:px-8 lg:px-10 py-16 sm:py-20 overflow-hidden"
    >
      {/* Heading */}
      <FadeIn delay={0} y={40}>
        <h2
      ...
      {/* Animated bio text */}
      <div className="mt-8 sm:mt-14 lg:mt-16" style={{ fontSize: 'clamp(0.9rem, 2vw, 1.35rem)' }}>
        <AnimatedText
          text={BIO_TEXT}
      ...
      {/* About cards */}
      <div className="mt-12 sm:mt-20 lg:mt-24 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 max-w-6xl w-full">
        {ABOUT_CARDS.map((card, i) => (
          <FadeIn key={card.id} delay={i * 0.15} y={30}>
            <div
              className="theme-dark-surface rounded-[24px] sm:rounded-[40px] lg:rounded-[50px] border-2 border-[var(--theme-border)] p-5 sm:p-8 lg:p-10 h-full flex flex-col gap-4 transition-colors duration-300 hover:border-[var(--theme-hover)]"
            >
              <card.icon
              ...
              <h3
                ...
                style={{ fontSize: 'clamp(1.1rem, 2.5vw, 2.25rem)' }}
              >
                {card.title}
              </h3>
              <p
                className="theme-secondary-text font-light leading-relaxed"
                style={{ fontSize: 'clamp(0.85rem, 1.4vw, 1.1rem)', opacity: 0.8 }}
              >
                {card.description}
              </p>
            </div>
      ...
      {/* Why Choose AGHSAN */}
      {showWhyChoose && (
      <div className="mt-20 sm:mt-28 lg:mt-32 max-w-6xl w-full">
        <FadeIn delay={0} y={40}>
          <h2
            className="hero-heading font-black uppercase leading-none tracking-tight text-center mb-10 sm:mb-16 lg:mb-20"
        ...
        <div className="flex flex-col gap-6 sm:gap-10">
          {WHY_CHOOSE.map((item, i) => (
            <FadeIn key={item.number} delay={i * 0.1} y={20}>
              <div
                className="theme-dark-surface rounded-[24px] sm:rounded-[40px] lg:rounded-[50px] border-2 border-[var(--theme-border)] p-6 sm:p-10 lg:p-12 flex flex-col gap-4 transition-colors duration-300 hover:border-[var(--theme-hover)]"
              >
                <span
                ...
                <p
                  ...
                  style={{ fontSize: 'clamp(0.85rem, 1.4vw, 1.15rem)', opacity: 0.8 }}
                >
                  {item.description}
                </p>
              </div>
